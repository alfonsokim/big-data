...
cat ufo-tiempo-seg.tsv | awk '{FS="|"} {if(min==""){min=max=$1}; if($1>max){max=$1}; if($1<min){min=$1}; total+=$1; count++} END {print "media = " int(total/count), "; minimo = " min, "; maximo = " max, "; n = " count, ";suma = " total}'>estadisticas.txt
...
cat ufo-tiempo-seg.tsv | ./estadistica_ufo.py > estadisticas_py.txt

#Resultados con R
cat ufo-tiempo-seg.tsv | ./estadistica_ufo.r > estadisticas_r.txt

#Con Python me dan diferentes: total, n, media, desviacion estandar y varianza :(

#Con R me dan diferentes: desviacion estandar y varianza :(
